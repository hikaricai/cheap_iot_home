...
public class AccessoryEngine {
    ...
    public static final byte CMD_HT = 0x00;
    public static final byte CMD_LED = 0x01;
    ...
    public AccessoryEngine(Context applicationContext, IEngineCallback callback) {
        ...
        L.d("construct accessory engine");
    }

    public void onNewIntent(Intent intent) {
        L.d("accessory engine onNewIntent");
        if (mUsbManager.getAccessoryList() != null) {
            L.d("accessory list is not null");
    ...
    private void connect() {
        if (mAccessory != null) {
            if(!mUsbManager.hasPermission(mAccessory)){
                ...
                L.d("try to get permission");
                return;
            }
            if (sAccessoryThread == null) {
                L.d("try to start thread");
    ...
    private final BroadcastReceiver mPermissionReceiver = new BroadcastReceiver(){
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            L.d("get permision receiver");
            if(intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)){
                L.d("has permison try to connect");
                connect();
            }
        }
    };
